...
def onsets_vec(events, sfreq):
    "make onset vectors from event matrix"
    ...
    e_timed[:, 0] = (e_timed[:, 0] - first_event) / sfreq 
    ...
    return (t_light, t_button, t_task)
...
## Dataframe
def mk_iti_ttl(task_onsets, t_task):
    ...
    return (ttl_df, iti_ttl, iti_task_trunc)

def iti_info(iti_task, ttl_df, iti_ttl, iti_task_trunc):
    "trunc not always the correct size"
...
def plot_task(task_onsets,
              t_light, t_button, t_task,
              ttl_df, iti_task_trunc):
    ## plot
    plt.close()
...
def measure_offsets(t_light, iti_ttl):
    #### OFFSET
...
def main(bdf, tsv):
    ...
    task_onsets = read_task(tsv)
    (t_light, t_button, t_task) = onsets_vec(events, info["sfreq"])
    (ttl_df, iti_ttl, iti_task_trunc) = mk_iti_ttl(task_onsets, t_task)
    plot_task(task_onsets, t_light, t_button, t_task,
              ttl_df, iti_task_trunc)


if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="plot habit run")
    parser.add_argument("bdf",help="eeg file")
    parser.add_argument("tsv",help="task file tab sep (not json)")
    args = parser.parse_args()
    print(args)

    plt.ioff()
    main(args.bdf, args.tsv)
    plt.show()
